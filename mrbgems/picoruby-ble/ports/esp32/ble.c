...
// NimBLE ends the characteristics and descriptors arrays with a zeroed entry, so
// finishing a service or a characteristic also takes a slot -- hence the calls
// whose result is only checked for exhaustion.
