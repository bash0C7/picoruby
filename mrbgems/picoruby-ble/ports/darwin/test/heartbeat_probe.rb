...
#
# mrblib/ble.rb's start loop calls `heartbeat_callback if pop_heartbeat` every
# POLLING_UNIT_MS, and pop_heartbeat only returns true when a port has called
# BLE_heartbeat(). rp2040 drives that from a btstack timer and the esp32 port
# from an esp_timer; this probe measures whether the darwin port drives it at
# all. Expected on a port that does: RUN_MS / HEARTBEAT_PERIOD_MS ticks.
#
